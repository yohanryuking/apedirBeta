...
import { Modal, BottomNavigation, BottomNavigationAction, Box, Menu, MenuItem } from '@mui/material'; // Componentes de Material UI
...
const Home = () => {
  ...
  const handleChange = (event, newValue) => {
    if (newValue !== 4) { // Si el nuevo valor no es el índice del Avatar
      if (newValue == 2) { // Si el nuevo valor no es el índice del Avatar
        handleOpen();
      } else {
        setValue(newValue);
      }
  ...
  const handleCloseModal = () => setOpen(false); // Función para cerrar el modal
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', paddingBottom: '75px' }}>
      ...
      <BottomNavigation value={value} onChange={handleChange} sx={{ width: '90%', position: 'fixed', bottom: 15, borderRadius: '30px', boxShadow: '0px 0px 10px rgba(0, 0, 0, 0.5)' }}>
        ...
        <BottomNavigationAction sx={{ minWidth: 'auto', padding: isMobile ? '6px 0' : '6px 12px' }} icon={<SearchIcon style={{ fontSize: isMobile ? '2em' : '2.5em' }} />} />
        ...
        <BottomNavigationAction
          sx={{ minWidth: 'auto', padding: isMobile ? '6px 0' : '6px 12px' }}
          icon={
            <Avatar
              src={'https://duerpqsxmxeokygbzexa.supabase.co/storage/v1/object/public/images/' + user + '/avatar.jpg'}
              style={{ fontSize: isMobile ? '1em' : '1.5em', border: '2px solid purple' }}
              onClick={handleClick}
            />
          }
        />
        ...
        >
          ...
          <MenuItem onClick={() => { setValue(2) }}>carrito</MenuItem>
